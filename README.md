...
#### 设计哲学：同构分片 + 页脚自描述 + dpbin 差集

- `pbin` 与 `fpbin`、dpbin 采用**完全相同的物理格式**，只是生命周期和命名前缀不同。
- 已完成的分片是**自描述对象**：文件末尾自带 `Footer`，记录实际行数与校验信息，不再需要外部索引。
- dpbin（目录完成日志）记录本次会话已完成的目录指纹，与 pbin 求差集得到续传边界。
