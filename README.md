# listfiles

> 高性能递归文件系统扫描器，支持智能断点续传、半增量扫描与设备熔断机制。

## 功能概览

`listfiles` 是一个面向 Linux 的底层文件系统遍历工具，旨在解决大规模（数亿文件级）目录树扫描中的几个核心痛点：

- **断点续传**：扫描过程中断后（含恢复过程再次崩溃），可从上次位置恢复，续传结果与干净全量扫描逐行一致，无需重新开始。
- **半增量扫描**：基于历史记录的 `blind-trust` 机制，对基准中已存在且长期未变更的条目跳过 `lstat` 调用，显著降低 I/O 开销。v15.6.0 起工具强制校验前提：必须 `--reference-base` 显式指定合格基准，且本轮 `-f` 为新空进度目录，否则拒绝运行。
- **Run manifest 运行凭证**：每次运行原子写入 `{base}.manifest`，记录 status 与 `baseline_eligible`——只有完整、无残留、非盲信的运行才可作为下次恢复/盲信的基准，不完整运行不会链式污染。
- **明确的退出码语义**：`0`=完全完成；`1`=部分完成（有跳过/残留，结果不可作基准）；`2`=严重失败（基准不合格、进度不兼容等，需按提示处理）。
- **设备熔断**：当底层存储设备（如 NFS、 SAN LUN）无响应时，自动将对应设备加入黑名单并跳过，避免整个扫描进程陷入 D-State 不可杀死的阻塞状态。
- **设备渐进探测**：对熔断设备以指数退避策略定时探测，设备恢复后自动将积压任务重新入队。
- **多进程并发**：采用 `fork()` + `pipe` 的独立 Worker 进程模型，利用内核的 COW（写时复制）实现零拷贝共享只读上下文。

## 系统要求

- **Linux**（依赖 `fork`, `epoll`, `pipe2`, `pthread`）
- **GCC** 支持 GNU11 标准（`-std=gnu11`）
- **zlib** 开发库（`-lz`）
- **NFS 挂载建议**：如扫描 NFS 目录，务必使用 `soft,intr,timeo=600` 选项。D-State 线程无法从用户态杀死，`soft` 挂载是避免 Worker 进程永远挂起的唯一手段。

## 构建

```bash
make
```

可执行文件将生成在 `bin/listfiles`。清理构建产物：

```bash
make clean
```

## 基本用法

### 全量扫描

```bash
./bin/listfiles --path=/data
```

### 单文件目标扫描

```bash
./bin/listfiles --path=/data/single_file.txt
```

### 带断点续传的扫描

```bash
./bin/listfiles --path=/data --continue --progress-file=task1
```

中断后再次执行相同命令即可从上次位置恢复。`pbin` 分片保留已发现记录，`dpbin` 完成日志用于差集续传，无需外部索引。

> **注意（v15.6.0 破坏性变更）**：v15.6.0 之前生成的旧格式进度文件（无 manifest、pbin schema 1）不能续传、不能作为盲信基准。检测到旧残留时拒绝续传（退出码 2），必须加 `--runone` 重新全量扫描建立新基准。

### 半增量扫描

```bash
# 前提：已有一次合格基准运行（其 manifest 中 baseline_eligible=1）
# 本轮 -f 必须指定一个全新的空进度目录，基准经 --reference-base 显式指定
./bin/listfiles --path=/data --continue --skip-interval=604800 \
    --progress-file=task_blind --reference-base=task1
```

门禁任一不满足（未指定 `--reference-base`、基准不合格、`-f` 目录非空）即拒绝运行（退出码 2）。盲信运行的结果 `baseline_eligible=0`，不能链式作为下次盲信的基准。

### CSV 输出

```bash
./bin/listfiles --path=/data --csv --output=files.csv
```

### 自定义格式

```bash
./bin/listfiles --path=/data --format="%p\t%s\t%u\t%g\t%m"
```

### 按元数据开关输出

不指定 `--format` 时，可通过独立开关动态控制默认文本格式：

```bash
./bin/listfiles --path=/data --size --user --mtime
# 输出格式自动变为: path|size|user|mtime
```

## 命令行选项

| 选项 | 说明 |
|------|------|
| `-p, --path=路径` | 要扫描的目标路径（**必需**，支持目录、普通文件或符号链接） |
| `-c, --continue` | 启用智能续传 / 增量模式 |
| `--runone` | 强制全量扫描（忽略历史进度；旧格式进度不兼容时必须使用） |
| `-y, --yes` | 跳过启动时的交互式确认 |
| `--skip-interval=秒` | 半增量扫描的时间阈值（默认：0；启用时须配合 `--reference-base`，见上半增量扫描节） |
| `--reference-base=前缀` | 盲信扫描的历史基准进度前缀（须 `baseline_eligible=1` 且 pbin schema 2，v15.6.0 起强制） |
| `--worker-count=数量` | Worker 进程数（默认：自动，上限：64，超限自动钳制并告警） |
| `--batch-size=数量` | Worker 每批发送的文件数（默认：1024） |
| `--estimated-files=数量` | 预估文件数，用于 HashSet 预分配（默认：10,000,000） |
| `-t, --timeout=秒` | Worker 心跳超时时间（默认：30） |
| `-f, --progress-file=前缀` | 进度文件前缀（默认：`progress`） |
| `-o, --output=文件` | 结果输出文件（默认：`output.txt`） |
| `-O, --output-split=目录` | 按行分片输出到目录 |
| `--csv` | 启用标准 CSV 输出格式 |
| `-Q, --quote` | 对输出字段进行引号包裹 |
| `-D, --dirs` | 在输出中包含目录本身的信息 |
| `-d, --print-dir` | 将当前扫描目录打印到标准错误 |
| `-F, --format=格式` | 自定义输出格式模板 |
| `--size, --user, --group, --mtime, --atime, --mode, --xattr` | 输出对应元数据（动态影响默认文本格式，不与 `--format` 同时生效） |
| `--master-threads=数量` | Master CPU 去重线程数（默认：4） |
| `--follow-symlinks` | 跟踪符号链接（递归遍历指向目录的符号链接） |
| `-M, --mute` | 禁用监控面板和诊断日志（`[System]`、`--verbose` 等），扫描数据正常输出。当不使用 `-o`/`-O` 而靠 stdout 管道化数据时，必须附加此参数。 |
| `-Z, --archive` | 将已处理的进度分片压缩归档 |
| `-C, --clean` | 删除已处理的进度分片（不与 `-Z` 同时使用） |
| `-R, --resume-from=文件` | 仅从指定进度列表文件恢复（**预留，暂未实现**） |
| `-v, --verbose` | 启用详细日志 |
| `--verbose-version=TIMESTAMP` | 只显示版本号 `>= TIMESTAMP` 的日志。默认值为当前编译版本号 `VERSION_CODE`，旧版本引入的调试日志默认被遮蔽。与 `--verbose` 配合使用，传 `0` 显示所有日志 |
| `-h, --help` | 显示帮助信息 |

### 格式说明符参考

`--format` 支持以下 `%` 前缀的说明符：

| 说明符 | 含义 | 示例 |
|--------|------|------|
| `%p` | 路径 | `/data/file.txt` |
| `%s` | 大小（字节） | `4096` |
| `%u` | 用户名 | `root(0)` |
| `%g` | 组名 | `root(0)` |
| `%U` | UID | `0` |
| `%G` | GID | `0` |
| `%m` | 修改时间 | `2024-01-15 08:30:00` |
| `%a` | 访问时间 | `2024-01-15 08:30:00` |
| `%c` | 变更时间 | `2024-01-15 08:30:00` |
| `%i` | Inode 号 | `142408` |
| `%o` | 权限字符串 | `-rw-r--r--` |
| `%O` | 权限八进制 | `0644` |
| `%t` | 文件类型 | `FILE` / `DIR` / `LINK` |
| `%X` | 扩展属性（lsattr） | `----------------` |

## 测试

```bash
tests/run_regression.sh        # 回归测试集（用例 1-17，含子用例共 21 个断言点：
                               # 基线一致性、并发一致性、错误注入、崩溃续传、
                               # 二次崩溃、盲信门禁、kill -9 Worker spare 补位、
                               # 全灭+spare 耗尽看门狗、EINVAL 非法文件名治理等；
                               # 通过标准为 PASS=21 FAIL=0）
```

## 版本

当前版本：**15.6.2**

## 许可证

**No License**

本软件及其全部源代码、文档、设计资料、二进制文件（以下统称"本软件"）的所有权利均归作者保留。未经作者事先书面许可，任何个人或实体不得以任何形式使用、复制、修改、合并、发布、分发、再许可、销售、逆向工程、反编译或反汇编本软件的全部或任何部分。

**保留权利（All Rights Reserved）**
- 著作权、专利权、商标权及其他知识产权均归作者所有。
- 本软件不提供任何形式的许可授权。

**免责声明（Disclaimer）**
本软件按"原样"（AS IS）提供，不作任何明示或暗示的担保，包括但不限于对适销性、特定用途适用性、非侵权性的担保。在任何情况下，作者均不对因使用或无法使用本软件而引起的任何直接、间接、附带、特殊、惩罚性或后果性损害承担责任，即使已被告知该等损害的可能性。

**严正警告（Warning）**
- 未经授权使用、复制、传播本软件即构成侵权。
- 任何试图绕过、移除或篡改本软件中的技术保护措施的行为均属违法。
- 作者保留对任何侵权行为追究法律责任的一切权利。
