...
**`Monitor`** 是独立的监控线程，每 500ms 刷新一次统计面板（输出到 **stdout**），内容包括：运行时间、活跃 Worker 数、待处理任务数、目录/文件/消费速率、输出进度、设备状态（死设备/判死设备数）、探测状态等。监控线程同时负责 Worker 心跳超时检查和敢死队探测的调度与收割，使主循环专注处理 IPC 消息。
