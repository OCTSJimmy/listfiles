...
### P0-003 [NEW] fpbin 二次崩溃恢复
- **问题**：第一次续传期间发现新目录 A 写入 fpbin，旧 pbin 未消费完时再次崩溃。第二次续传不读 fpbin，A 永久漏扫
- **根因**：fpbin 崩溃语义未定义
- **方向**：
  1. 恢复时读取 fpbin：`待扫描 = (pbin-dpbin) ∪ fpbin ∪ dspill ∪ spbin_recoverable`
  2. 转正原子化：旧 pbin 消费完 → fpbin 重命名为 pbin.{next_seq} → 更新 pump cursor
  3. fpbin 格式与 pbin 一致（文本 + Footer）
- **评审来源**：外部评审 P0-3

### P0-004 [NEW] visited_set 背压竞态
- **问题**：目录因背压写 pbin 后进 visited_set，但未入队，只进 dspill。回填时 visited_set 误判"已访问"而丢弃
- **根因**："已发现"等价于"已入队"
- **方向**：visited_set 语义拆分为三态：
...
### P0-008 [NEW] Run manifest + baseline_eligible 原子切换
...
### P1-009 [NEW] pbin 文本格式 → 二进制安全编码
- **问题**：Linux 文件名可含换行/控制字符，pbin 一行一条记录模型被破坏
- **方向**：
  1. 持久化记录采用长度前缀编码（与 IPC BATCH 的 path_len 一致）
  2. 或定义明确转义规则（如 CSV 风格转义）
  3. 推荐长度前缀，因为零歧义、解析快
- **评审来源**：外部评审
...
| 编号 | 问题 | 位置 | 修正方向 |
|------|------|------|---------|
| DOC-001 | IPC 消息方向表错误：§7.2 混了 M→W 和 W→M | §7.2 | 拆成两个方向表 |
| DOC-002 | pbin 写入者前后不一致：§4.1 vs §5.1 | §4.1 / §5.1 | 明确 batch_processor 写 pbin，async_worker 只写输出 |
| DOC-003 | "pbin 记录所有扫描过的条目"应为"已发现" | §8.1 | 修正语义 |
| DOC-004 | Monitor 与 Main 进程收割职责重复 | §5.1 / §5.4 | 明确唯一 owner |
| DOC-005 | "新文件/变更文件输出"与盲信语义矛盾 | §4.3 | 盲信模式下不存在"变更文件" |
| DOC-006 | "减少 90%+ I/O"没有论证 | §10 | 删除或补模型/测试数据 |
| DOC-007 | `timeo=600` 单位错误（应为 6000=600秒） | §12 | 修正挂载参数 |
| DOC-008 | `intr` 在 CentOS 7.4 无效，不应作为关键前提 | §12 | 移除 intr |
...
> 注："新增"指上一版评审（Jimmy 2026-08-09）未触及；"升级"指上一版已触及但本版深化或方案变更
