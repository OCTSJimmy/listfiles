        ...
        if (S_ISDIR(st->st_mode)) {
            if (ctx->hist_pump_state == HIST_PUMP_OLD) {
                ...
                if (rc == -1) {
                    ...
                    /* Worker pipe broken: decrement pending_tasks and queue for redispatch */
                    atomic_fetch_sub(&ctx->pending_tasks, 1);
                    if (ctx->lost_count >= ctx->lost_capacity) {
                        size_t new_cap = ctx->lost_capacity ? ctx->lost_capacity * 2 : 64;
                        char **new_arr = realloc(ctx->lost_tasks, new_cap * sizeof(char *));
                    ...
                    if (ctx->lost_count < ctx->lost_capacity) {
                        ctx->lost_tasks[ctx->lost_count++] = strdup(path);
                    } else {
                        fprintf(stderr, "[Warning] Lost task queue full, dropping %s\n", path);
                    }
                }
...
static void flush_worker_backlogs(AppContext *ctx) {
    for (int i = 0; i < ctx->worker_pool->num_workers; i++) {
        ...
        for (int j = 0; j < slot->backlog_count; j++) {
            ...
            if (rc == -1) {
                /* Worker pipe broken, keep task in queue for next round */
                fprintf(stderr, "[Backlog] ipc_send to worker %d failed, keeping %s\n", i, path);
                continue;
            }
            free(path);
...
static void dispatch_lost_tasks(AppContext *ctx) {
    ...
    for (size_t i = 0; i < ctx->lost_count; i++) {
        ...
        if (rc == -1) {
            /* Worker pipe broken, keep task in queue for next round */
            fprintf(stderr, "[LostTasks] ipc_send to worker %d failed, keeping %s\n", wid, path);
            continue;
        }
...
void main_loop_run(AppContext *ctx) {
    ...
    while (ctx->running) {
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            /* Handle worker death events (EPOLLERR/EPOLLHUP) before reading */
            ...
            int rc_hdr = ipc_recv_header(ctx->worker_pool->slots[slot_id].fd_out, &hdr);
            if (rc_hdr == -2) {
                /* EAGAIN: spurious wake or fd reused before data available, skip */
                continue;
            }
            if (rc_hdr != 0) {
